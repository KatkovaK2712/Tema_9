...
3) Создайте метод init (), внутри которого будут определены два динамических свойства: _index (передается параметром) и _state    (принимает первое значение из словаря states).
После написания этого блока кода в комментарии к нему укажите какими являются эти два свойства:
...
## Выводы
Код определяет класс Tomato, который моделирует процесс созревания помидора, предоставляя методы для перехода к следующей стадии и проверки зрелости.
...
## Выводы
Класс TomatoBush предоставляет методы для управления и отслеживания роста и созревания помидоров в кусте.
...
## Выводы
Класс Gardener представляет собой модель садовника, который может ухаживать за растениями и собирать урожай, а также предоставляет общие знания о садоводстве.
